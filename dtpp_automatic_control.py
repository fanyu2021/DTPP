...
from agents.navigation.dtpp_agent import DtppAgent  # pylint: disable=import-error
...
class World(object):
    ...
    def restart(self, args):
        ...
        # print(f'--- blueprint_list:{blueprint_list}')
        # 从列表中筛选出 满足条件的 blueprint
        # blueprint = list(filter(lambda blp: blp.id == "vehicle.tesla.model3", blueprint_list))[0]
        # 找到第一个满足条件的 blueprint
...
def game_loop(args):
    ...
    try:
        ...
        if args.agent == "Basic":
            ...
            agent = ConstantVelocityAgent(world.player, 30, opt_dict)
        ...
        while True:
            ...
            if not args.sync:
            # if True:
                logger.info('Sleep time: %.2f seconds' %(sleep_time))
...
def get_argparser():
    ...
    argparser.add_argument(
        "-a", "--agent", type=str,
        choices=["Behavior", "Basic", "Constant", "Dtpp"],
